...
cast = jaxutils_teacher.cast_to_compute
...
class RSSM(nj.Module):
    ...
    def observe(self, embed, action, is_first, state=None):
        ...
        start = state, state
        post, prior = jaxutils_teacher.scan(step, inputs, start, self._unroll)
    ...
    def imagine(self, action, state=None):
        ...
        action = swap(action)
        prior = jaxutils_teacher.scan(self.img_step, action, state, self._unroll)
        prior = {k: swap(v) for k, v in prior.items()}
        return prior
    ...
    def get_dist(self, state, argmax=False):
        if self._classes:
            ...
            return tfd.Independent(jaxutils_teacher.OneHotDist(logit), 1)
        else:
...
class MultiEncoder(nj.Module):
    ...
    def __call__(self, data):
        ...
        if self.mlp_shapes:
            ...
            inputs = jaxutils_teacher.cast_to_compute(inputs)
...
class MultiDecoder(nj.Module):
    ...
    def _make_image_dist(self, name, mean):
        ...
        if self._image_dist == "mse":
            return jaxutils_teacher.MSEDist(mean, 3, "sum")
...
class ImageEncoderResnet(nj.Module):
    ...
    def __call__(self, x):
        ...
        depth = self._depth
        x = jaxutils_teacher.cast_to_compute(x) - 0.5
        # print(x.shape)
...
class ImageDecoderResnet(nj.Module):
    ...
    def __call__(self, x):
        ...
        x = jaxutils_teacher.cast_to_compute(x)
...
class MLP(nj.Module):
    ...
    def __call__(self, inputs):
        ...
        if self._symlog_inputs:
            feat = jaxutils_teacher.symlog(feat)
        x = jaxutils_teacher.cast_to_compute(feat)
...
class Dist(nj.Module):
    ...
    def inner(self, inputs):
        ...
        if self._dist == "symlog_mse":
            return jaxutils_teacher.SymlogDist(out, len(self._shape), "mse", "sum")
        if self._dist == "symlog_disc":
            return jaxutils_teacher.DiscDist(out, len(self._shape), -20, 20, jaxutils_teacher.symlog, jaxutils_teacher.symexp)
        if self._dist == "mse":
            return jaxutils_teacher.MSEDist(out, len(self._shape), "sum")
        ...
        if self._dist == "onehot":
            if self._unimix:
                ...
                probs = (1 - self._unimix) * probs + self._unimix * uniform
                out = jnp.log(probs)
            dist = jaxutils_teacher.OneHotDist(out)
...
class Conv2D(nj.Module):
    ...
    def _layer(self, x):
        if self._transp:
            ...
            kernel = jaxutils_teacher.cast_to_compute(kernel)
        ...
        if self._bias:
            ...
            bias = jaxutils_teacher.cast_to_compute(bias)
            x += bias
        return x
