...
class _PolicyState:
    ...
    def to_host(self):
        return tree_map(np.asarray, jax.device_get(self.value))


def _policy_state_flatten(state):
    children, aux = tree_flatten(state.value)
    return children, aux


def _policy_state_unflatten(aux, children):
    return _PolicyState(tree_unflatten(aux, children))


jax.tree_util.register_pytree_node(_PolicyState, _policy_state_flatten, _policy_state_unflatten)
...
class JAXAgent(embodied.Agent):
    ...
    def train(self, data, state=None):
        ...
        if self._once:
            self._once = False
            assert jaxutils_teacher.Optimizer.PARAM_COUNTS
            for name, count in jaxutils_teacher.Optimizer.PARAM_COUNTS.items():
    ...
    def _setup(self):
        ...
        jaxutils_teacher.COMPUTE_DTYPE = getattr(jnp, self.config.precision)
