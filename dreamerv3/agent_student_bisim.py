...
class Agent(nj.Module):
    ...
    def train(self, data, teacher_data, state, teacher_state,traj=None, teacher_traj=None):
        ...
        state2, teacher_state2, wm_outs2, mets2 = self.wm.train(
            data, teacher_data, state, teacher_state, traj=sg(traj), teacher_traj=sg(teacher_traj))
...
class WorldModel(nj.Module):
    ...
    def loss(self,traj,teacher_traj, data, teacher_data, state,teacher_state):
        ...
        if (traj is not None) and (teacher_traj is not None):
            ...
            t_rew = sg(self.teacher_wm.heads["reward"](teacher_traj).mean())
            # Align time dims to [T,B] (exclude last latent if needed)
            if s_rew.shape[0] == teacher_traj["action"].shape[0] + 1:
                s_rew = s_rew[:-1]
        ...
        if (traj is not None) and (teacher_traj is not None):
            ...
            # ------------------------------------------------------------------
            # Pairwise φ-based bisimulation target under STUDENT actions
            # ------------------------------------------------------------------

            # 1) φ-gap on STUDENT current latents at time t (align to actions):
            # traj has [T+1,B,...] latents and [T,B,A] actions -> use [: -1] for time t.
            def _phi_norm(feat):
                z = self.phi_head(feat).mean()  # uses 'deter' by config
                return z / (jnp.linalg.norm(z, axis=-1, keepdims=True) + 1e-6)

            s_now = {k: v[:-1] for k, v in traj.items() if k in ("deter", "stoch", "logit")}
            ...
            # 2) Teacher reward difference at time t (teacher under SAME actions):
            t_rew = self.teacher_wm.heads["reward"](jax.tree_map(sg, teacher_traj)).mean()
            # Align to [T,B]
            if t_rew.ndim == 3 and t_rew.shape[-1] == 1:
                t_rew = t_rew[..., 0]
            if t_rew.shape[0] == teacher_traj["action"].shape[0] + 1:
                t_rew = t_rew[:-1]  # drop last state to match actions length
            dR = jnp.abs(t_rew - jnp.take(t_rew, perm, axis=1))  # [T,B]

            # 3) Teacher transition difference at NEXT time (t+1) under SAME actions:
            # Option A (simple): φ distance on teacher NEXT latents, with STOP-GRAD.
            def _phi_norm_T_no_grad(featT):
                z = self.phi_head(featT).mean()
                z = jax.lax.stop_gradient(z)
                return z / (jnp.linalg.norm(z, axis=-1, keepdims=True) + 1e-6)
            ...
            trans_gap = jnp.linalg.norm(_phi_norm_T_no_grad(t_next_i)
                                        - _phi_norm_T_no_grad(t_next_j), axis=-1) # [T,B]

            # (Option B: symmetric KL between teacher NEXT distributions)
            # td_i = self.teacher_wm.rssm.get_dist(t_next_i)
            # td_j = self.teacher_wm.rssm.get_dist(t_next_j)
            # kl_ij = distrax.kl_divergence(td_i, td_j)
            # kl_ji = distrax.kl_divergence(td_j, td_i)
            # if kl_ij.ndim > 2:
            #     rdims = list(range(2, kl_ij.ndim))
            #     kl_ij = kl_ij.mean(rdims); kl_ji = kl_ji.mean(rdims)
            # trans_gap = 0.5 * (kl_ij + kl_ji)  # [T,B]

            # 4) Build the bisimulation target dT = α |ΔR| + γ * transition_gap
            alpha_r     = float(getattr(self.config, "bisim_alpha_r", 1.0))
            gamma_bisim = float(getattr(self.config, "bisim_gamma",    0.99))
            dT = alpha_r * dR + gamma_bisim * trans_gap  # [T,B]

            # 5) Mask for imagined steps (use teacher continuation as weight, optional)
            ...
            if t_cont.shape[0] == teacher_traj["action"].shape[0] + 1:
                t_cont = t_cont[:-1]
            mask = t_cont.astype(jnp.float32)  # [T,B]; you can also use jnp.ones_like(dT)

            # 6) Calibrate/normalize target with EMA and compute Huber loss
            batch_mean = (jnp.sum(dT * mask) / (jnp.sum(mask) + 1e-6))
            ...
            # 7) Metrics for debugging/monitoring
            wsum = jnp.sum(mask) + 1e-8
            x_mean = jnp.sum(mask * phi_now_gap) / wsum
            y_mean = jnp.sum(mask * dT) / wsum
            x_std  = jnp.sqrt(jnp.sum(mask * (phi_now_gap - x_mean)**2) / wsum + 1e-8)
            y_std  = jnp.sqrt(jnp.sum(mask * (dT        - y_mean)**2) / wsum + 1e-8)
            cov_xy = jnp.sum(mask * (phi_now_gap - x_mean) * (dT - y_mean)) / wsum
