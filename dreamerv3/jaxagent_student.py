...
class JAXAgent(embodied.Agent):
    def __init__(self, agent_cls, obs_space, act_space, teacher_wm, teacher_policy, step, config):
        ...
        self._setup()
        self.agent = agent_cls(obs_space, act_space, teacher_wm, teacher_policy, step, config, name="agent")
    ...
    def train(self, data, teacher_data, state=None,teacher_state=None,traj=None, teacher_traj=None):
        # print("yes1")
        ...
        if teacher_state is None:
            teacher_state, _ = self._init_train(self.varibs, rng, teacher_data["is_first"])
        (traj, teacher_traj,outs, state, teacher_state,mets), self.varibs = self._train(self.varibs, rng, data, teacher_data, state,teacher_state,traj=traj, teacher_traj=teacher_traj)
        ...
        if self._once:
            self._once = False
            assert jaxutils_student.Optimizer.PARAM_COUNTS
            for name, count in jaxutils_student.Optimizer.PARAM_COUNTS.items():
                if count is not None:
                    mets[f"params_{name}"] = float(count)
        # return outs, state, mets
        return traj, teacher_traj, outs, state,teacher_state, mets

    def report(self, data,teacher_data):
        rng = self._next_rngs(self.train_devices)
        mets, _ = self._report(self.varibs, rng, data,teacher_data)
        mets = self._convert_mets(mets, self.train_devices)
        return mets
    ...
    def sync(self):
        ...
        if len(self.policy_devices) == 1:
            ...
            self.policy_varibs = jax.device_put_replicated(varibs, self.policy_devices)
    ...
    def _setup(self):
        ...
        jaxutils_student.COMPUTE_DTYPE = getattr(jnp, self.config.precision)
