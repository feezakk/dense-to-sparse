...
class TeacherObs(gym.ObservationWrapper):
...
class StudentObs(gym.ObservationWrapper):
    def __init__(self, env):
        super().__init__(env)
        self.observation_space = spaces.Dict({
            "image": env.observation_space["image"],
...
class CollectorObs(gym.ObservationWrapper):
    """Expose BOTH views for logging/distillation datasets."""
    def __init__(self, env):
        super().__init__(env)
