...
from . import agent_teacher, expl_teacher, jaxutils_teacher
...
class Greedy(nj.Module):
    def __init__(self, wm, act_space, config):
        ...
        if config.critic_type == "vfunction":
            critics = {"extr": agent_teacher.VFunction(rewfn, config, name="critic")}
        else:
            raise NotImplementedError(config.critic_type)
        self.ac = agent_teacher.ImagActorCritic(critics, {"extr": 1.0}, act_space, config, name="ac")
...
class Random(nj.Module):
    ...
    def policy(self, latent, state):
        ...
        if self.act_space.discrete:
            dist = jaxutils_teacher.OneHotDist(jnp.zeros(shape))
        else:
...
class Explore(nj.Module):
    ...
    def __init__(self, wm, act_space, config):
        ...
        for key, scale in config.expl_rewards.items():
            ...
            if key == "extr":
                rewfn = lambda s: wm.heads["reward"](s).mean()[1:]
                critics[key] = agent_teacher.VFunction(rewfn, config, name=key)
            else:
        ...
        self.ac = agent_teacher.ImagActorCritic(critics, scales, act_space, config, name="ac")
